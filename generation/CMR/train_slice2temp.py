...
# from model.models import DiT_models
from model.models_Slice2Temp import DiT_models
# from CMRECGDataset_Latent import *
from CMRECGDataset_Slice2Temp import *
from utils_slice2temp import *
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in progress_bar:
            ...
            z_slice = batch["latent_slice"].to(args.device)
            time_mask = batch["time_mask"].to(args.device)
            t_idx = batch["t_given"].to(args.device)
            slice_idx = batch["slice_idx"].to(args.device)

            x_mri = x_mri * scale_factor
            z_slice = z_slice * scale_factor
            ...
            c = (z_slice, slice_idx, t_idx, z_ecg, keyframe_q)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--min_lr', type=float, default=1e-5, help='lower lr bound for cyclic schedulers that hit 0')
    ...
    parser.add_argument('--loss_weight_modeA', type=float, default=4.0, help='Loss weight multiplier for Mode A (keyframe generation)')
    parser.add_argument('--loss_weight_modeB', type=float, default=1.0, help='Loss weight multiplier for Mode B (interpolation)')
