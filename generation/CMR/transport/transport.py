...
class Transport:
    ...
    def training_losses(
        ...
        def masked_mean(sq_err, mask):
            if mask is None:
                return mean_flat(sq_err)
            m = mask
            while m.dim() < sq_err.dim():
                m = m.unsqueeze(-1)
            num = (sq_err * m).sum(dim=tuple(range(1, sq_err.dim())))
            den = m.sum(dim=tuple(range(1, m.dim()))).clamp_min(1.0)
            return num / den  # [B]

        if self.model_type == ModelType.VELOCITY:
            sq_err = (model_output - ut) ** 2
            sq_err = apply_mask(sq_err, mask)
            terms["loss"] = mean_flat(sq_err)
        else:
            ...
            if self.model_type == ModelType.NOISE:
                sq_err = (model_output - x0) ** 2
            else:
                sq_err = (model_output * sigma_t + x0) ** 2

            per = weight * sq_err
            per = apply_mask(per, mask)
            terms["loss"] = mean_flat(per)
