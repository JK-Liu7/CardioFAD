...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.cond_proj_spatial = nn.Sequential(
            nn.SiLU(),
            nn.Linear(self.in_channels, hidden_size, bias=True)
        )
    ...
    def forward(self, 
        ...
        z_volume, t_index, z_ecg, keyframe_q = context

        z_volume_expanded = z_volume.expand(batches, frames, channels, high, weight, depths)
        x = torch.cat([x, z_volume_expanded], dim=2)
        ...
        z_volume_ = z_volume.mean(dim=(1, 3, 4, 5))                                  # [B, C]
        c_spatial = z_volume_       # [B, C]
